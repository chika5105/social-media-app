...
const comment = (params, credentials, postId, newComment) => {
  return fetch('/api/posts/comment/', {
    ...
    body: JSON.stringify({ userId: params.userId, postId, newComment }),
  }).then((response) => {
    return response.json();
  }).catch((err) => {
    console.log(err);
...
const uncomment = (params, credentials, postId, newComment) => {
